...
class SquarePinField : PinField{
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        for (i in 0 until numberOfFields){
            ...
            var textSizeConverter = TextSizeConverter(context)
            textPaint.textSize=textSizeConverter.getTextSize(44);
            ...
            if(character!=null) {
                val typeface = ResourcesCompat.getFont(context, R.font.pingmedium)
    ...
    public fun setDistance(distance: Float,radius: Float){
        distanceInBetween=distance
        cornerRadius=radius
        invalidate()
    }
}