            ...
            public void onClick(View view) {
                if (binding.edtEmailPhone.getText().toString().isEmpty()){
                    if (registerType.equalsIgnoreCase("phone")) {
                        SuccessFailureDialog.showFailureDialog(RegisterAccountActivity.this, textSizeConverter, getString(R.string.phone_failure), registerType, false, null);
                    }else{
                        SuccessFailureDialog.showFailureDialog(RegisterAccountActivity.this, textSizeConverter, getString(R.string.email_failure), registerType, false, null);
                    }
                }else {
                    String phoneEmail = "";
                    if (registerType.equalsIgnoreCase("phone")) {
...
//                callCompanyDomainAPI();
                            ...
                            SuccessFailureDialog.showFailureDialog(RegisterAccountActivity.this, textSizeConverter, "phoneemailfail", registerType, false, null);
...
//                        Toast.makeText(getApplicationContext(), "invalide", Toast.LENGTH_SHORT).show();
    ...
    private void setLayoutAndTextSize() {
        ...
        textSizeConverter.changeStatusBarColor(RegisterAccountActivity.this);
        ...
        binding.txtResendDesc.setText(getString(R.string.dont_account));
        binding.txtResend.setText(getString(R.string.login));
        ...
        binding.lytResend.setVisibility(View.GONE);
...
//            binding.edtEmailPhone.setText("sneha@nashid.io");
    ...
    private void callGetOTPAPI() {
        LoaderUtil.showLoader(RegisterAccountActivity.this, binding.customLoader.loaderContainer, textSizeConverter);
        ...
        APIClient.getClient(url,"").create(APIInterface.class).callOTP(binding.edtEmailPhone.getText().toString()).
                enqueue(new Callback<ResponseBody>() {
                    ...
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        logger.log("response:companu " + response.body());
                        LoaderUtil.hideLoader(RegisterAccountActivity.this, binding.customLoader.loaderContainer, textSizeConverter);
                        if (response.isSuccessful()) {
                            try {
                                ...
                                if (success) {
                                    ...
                                    logger.log("Registartion:otpToken: "+token);
    ...
    private void callCheckUserAPI() {
        ...
        APIClient.getClient(url,"").create(APIInterface.class).callCheckUser(binding.edtEmailPhone.getText().toString()).
                enqueue(new Callback<ResponseBody>() {
                    @Override
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        logger.log("response:check user " + response.body());
                        LoaderUtil.hideLoader(RegisterAccountActivity.this, binding.customLoader.loaderContainer, textSizeConverter);
                        if (response.isSuccessful()) {
                            try {
                                String jsonResponse = response.body().string();
                                ...
                                JSONObject jsonObject = new JSONObject(jsonResponse);
                                boolean success = jsonObject.getBoolean("success");
                                logger.log("Registartion:Check user: "+success);
                                if (!success) {
                                    SuccessFailureDialog.showFailureDialog(RegisterAccountActivity.this,textSizeConverter,"apifail",registerType,false,null);
                                }else{
                                    callGetOTPAPI();
                                }
                                // Now you have the raw JSON response as a string (jsonResponse)
...
//    }
    ...
    private void openOTPConfirmActivity() {
        Intent intent = new Intent(RegisterAccountActivity.this, OTPConfirmActivity.class);
